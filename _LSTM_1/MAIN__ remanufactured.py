...
class DataSetHandler:
    def __init__(self, parameters):
        ...
        self.question = False if 'model' in parameters and parameters['model'] else True
        self.epochs = int(parameters.get('epochs') or parameters.get('epoch') or DEFAULTS_EPOCHS)
        self.test_months = int(parameters.get('test_months') or parameters.get('test_month') or DEFAULTS_TEST_MONTHS)

    def create_data_set(self, data_sources):
        dfs = {}
        for name in data_sources:
            class_name = name.capitalize() + "Data"  # Convert to class name, e.g., "YahooData"
            try:
                instance = getattr(sys.modules[__name__], class_name)() # Create instance
                func_name = f"get_{name}_data" # Call method
                df_name = f"df_{name}"
                print(f"[bold blue]===> Includes: {df_name}[/bold blue]")
                dfs[df_name] = getattr(instance, func_name)()
        ...
        self.data_set = list(dfs.values())[0] # Start with the first dataframe
        self.data_set.index = pd.to_datetime(self.data_set.index) # Convert the index to datetime
        self.data_set.index = self.data_set.index.to_period('M')  # Convert the index to year-month
        for df in list(dfs.values())[1:]:  # Merge all other dataframes
            df.index = df.index.to_period('M')  # Convert the index to year-month
            self.data_set = self.data_set.merge(df, left_index=True, right_index=True, how='inner')
        # convert index back to datetime format with first day of the month
        self.data_set.index = self.data_set.index.to_timestamp()

        # find missing dates
        find_missing_dates(self.data_set)

        # plot
        if self.question:
            ask_to_plot("Do you want to plot of the MERGED dataframe? (yes/no):", {'data_set': self.data_set})

        # add indicators to the dataframe
        self.data_set = add_indicators(self.data_set)

        self.data_set.to_csv('before_shift.csv', index=True)

        # #################### SHIFT non "SPX" columns by one month ############################
        non_spx_columns = [col for col in self.data_set.columns if not col.startswith('SPX')]
        for col in non_spx_columns:
            self.data_set[col] = self.data_set[col].shift(2)
        ########################################################################################

        self.data_set.to_csv('after_shift.csv', index=True)

        # plot
        if self.question:
            ask_to_plot("Do you want to plot of the MERGED dataframe WITH INDICATORS? (yes/no):", {'data_set': self.data_set})

        # sort columns by name 
        self.data_set.sort_index(axis=1, inplace=True)

        # backfill missing values
        self.data_set.fillna(method='bfill', inplace=True)

        # fill NaNs with interpolated values
        print("\n\nNans in data_set before interpolation = ", self.data_set.isnull().sum().sum())
        self.data_set.interpolate(method='linear', inplace=True)
        print("Number of Nans after interpolation = ", self.data_set.isnull().sum().sum(), "\n\n")

        # apply Transformations
        train_size = int(len(self.data_set) - TEST_MONTHS) 
        self.data_set, self.final_train_values = transform_features(self.data_set, train_size)
        print("\n\nFinal train values: ", self.final_train_values)

        # plot
        if self.question:
            ask_to_plot("Do you want to plot of the MERGED dataframe WITH TRANSFORMED FEATURES? (yes/no):", {'data_set': self.data_set}, normalize=False)
        
        # normalize the dataframes
        print(f"[bold yellow]============> NORMALIZING DATAFRAMES <============[bold yellow]\n\n")
        self.original_max_values = self.data_set.max()
        self.original_min_values = self.data_set.min()
        scaler = MinMaxScaler()

        self.data_set = pd.DataFrame(scaler.fit_transform(self.data_set), columns=self.data_set.columns, index=self.data_set.index)
        print(f"[bold green]Original max values:\n\n[/bold green]", self.original_max_values)
        print(f"\n[bold green]Original min values:\n\n[/bold green]", self.original_min_values)

        # plot
        if self.question:
            ask_to_plot("Do you want to plot the FINAL (TRANSFORMED & NORMALIZED) data_set?:", {'data_set': self.data_set}, normalize=False) # should bbe normalized anyway
...
# *************************************************************************************************
#                          NASDAQ.COM (quandl.com) (NOT USED NOW)
# -------------------------------------------------------------------------------------------------
class NasdaqData:
...
class YahooData:
    ...
    def get_online_yahoo_data(self, start_date):
        # get data from yahoo
...
def create_the_model_V1(self):
    # Implement logic for creating and training model version 1
    pass

# You can also create more methods for other versions of the model
# def create_the_model_V2(self):
#     pass
...
def create_and_train(self, model_choice):
    if model_choice == '1':
        model, X_test, y_test, dates_test = self.create_the_model_V1()
    # elif model_choice == '2':
...
def autocorrelation(self):
    # Implement the logic for autocorrelation tracing here
    pass
...
if __name__ == "__main__":
    data_sources = ["fred", "elections", "generator", "static"] # "yahoo" is not used anymore as I'm getting SPX from TadingView as a Static download
    parameters = parse_parameters(sys.argv[1:]) # param passed from command line

    data_handler = DataSetHandler(parameters)
    data_handler.create_data_set(data_sources)
    data_set = data_handler.data_set

    # ------------------------- OUTPUT -----------------------
    if data_handler.question:
    ...
    if model_choice == '1':
        model, X_test, y_test, dates_test = create_the_model_V1(data_handler.data_set, data_handler.epochs, data_handler.test_months)
    # elif model_choice == '2':
